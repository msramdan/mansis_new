<section class="content-header">
      <h1>Tambahan
        <small>Tambahan Benefit</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Tambahan</li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      <div class="box">
          <div class="box-header">
            <h3 class="box-title"><?=ucfirst($page)?> Tambahans</h3>
            <div class="pull-right">
              <a href="<?=site_url('tambahan')?>" class="btn btn-warning btn-flat">
          ...
          <div class="box-body ">
            <div class="row">
              <div class="col-md-4 col-md-offset-4">
                <form action="<?=site_url('tambahan/process')?>" method="post">
                  <div class="form-group ">
                    <label for="barcode">Nama Karyawan</label>
                  </div>
                  <div class="form-group input-group">
                    <input type="hidden" name="id_ramdan" value="<?=$row->tambahan_id?>">
                  ...
                  <div class="form-group ">
                    ...
                    <select name="benefit" class="form-control" required="">
                      <option value="">-- Pilih -- </option>
                      <?php foreach ($benefit->result() as $key => $data2) { ?>
                         <?php if ($row->benefit_id==$data2->benefit_id) { ?>
                        <option value="<?php echo $data2->benefit_id?>" selected><?php echo $data2->name ?></option>    
                        <?php }else{ ?>
                        <option value="<?php echo $data2->benefit_id?>"><?php echo $data2->name ?></option>      
                      <?php } ?>
                      <?php } ?>
                    </select>
                  </div>
                   ...
                   <div class="form-group ">
                    <label>Besar Tambahan</label>
                    <input type="number" name="besar_tambahan" value="<?=$row->besar_tambahan?>" class="form-control " required>
                  </div>
